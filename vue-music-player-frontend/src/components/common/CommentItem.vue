<template>
  <div class="flex">
    ...
    <div class="flex flex-col ml-4 justify-between mr-10">
      <!-- text -->
      <div>
        <span class="text-sky-600 text-main text-sm cursor-pointer">{{ props.comment.nickname }}：</span>
      ...
      <div class="items-center flex flex-1 flex-shrink-0">
        <IconPark
          ...
          @click="apiLikeComment(comment.commentId,userId)"
        />
        <IconPark
          v-else
          :icon="ThumbsUp"
          ...
          theme="filled"
          fill="#d0021b"
          @click="apiDelCommentLike(comment.commentId,userId)"
