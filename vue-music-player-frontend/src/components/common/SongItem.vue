<template>
    <div class="flex song-item items-center w-full hover-bg-main" :class="{'playing':id==propSong.songId}"
      ...
      <div class="flex-shrink-0 flex-1 flex items-center justify-between pr-5 ">
        ...
        <div class="items-center flex flex-1 w-10 flex-shrink-0">
          ...
          <IconPark v-if="true" class="ml-2 text-orange-400 cursor-pointer" size="16"
                    :icon="PlayTwo" @click="router.push({name:Pages.mvDetail,query:{id:propSong.mark}})"/>
        </div>

        <!-- 中间操作栏 -->
        <div class="hidden icon-action flex-shrink-0">
          <div class="flex items-center gap-x-1.5 text-gray-400 ml-2  ">
            <IconPark title="播放" :icon="PlayOne" size="20" class="hover-text" @click="play(propSong.songId)"/>
      ...
      <div class="flex-shrink-0" :class="{'w-1/4':showArName,'w-1/3':!showArName}" v-if="showAlName">
        <div class="w-9/12 truncate">
          <small class="truncate hover-text" @click="router.push({name:'album',query:{id:propSong.songId}})">{{
              propSong.name
            }}</small>
        </div>
      </div>
