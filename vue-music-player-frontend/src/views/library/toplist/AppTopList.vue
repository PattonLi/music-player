<template>
  ...
  <div class="grid grid-flow-row grid-cols-2 2xl:grid-cols-4 gap-5">
    <div v-for="item in topListData.slice(0,4)" :key="item.id"
         class="flex bg-dc rounded-lg items-center cursor-pointer" @click="toPlaylist(item)">
      <MyCover :name="item.name" :pic-url="item.coverImgUrl" :play-count="item.playCount" class="w-36 flex-shrink-0"
                 show-play-count/>
  ...
  <div class="grid grid-flow-row grid-cols-5 2xl:grid-cols-10 gap-5">
    <div v-for="item in topListData.slice(4)" :key="item.id" @click="toPlaylist(item)">
      <MyCover :name="item.name" :pic-url="item.coverImgUrl" :play-count="item.playCount"/>
...
const {topListData} = toRefs(useMusicStore());
const {updateTopListData} = useMusicStore();
const router = useRouter()
onMounted(async () => {
  await updateTopListData()
})
const toPlaylist = (topListDetail: TopList) => {
