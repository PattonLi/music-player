  ...
  <div class="container">
    ...
    >
      <div v-for="(item, index) in lyric" :key="index">
        ...
        <div>{{ _.findIndex(tlyric,(o)=>{return item.time==o.time})!=-1? tlyric[_.findIndex(tlyric,(o)=>{return item.time==o.time})].content : ''}}</div>
      </div>
    </n-carousel>
  </div>
</template>
...
const carousel = ref()
const { audio, lyric, tlyric, song } = storeToRefs(usePlayerStore())
