  ...
  <el-card class="main-card">
    <el-table :data="state.tableData" height="600" style="width: 100%">
      ...
      <el-table-column fixed="right" label="操作" width="200">
        <template #default="scope">
          <el-button type="warning" :icon="Edit" @click="EditClick(scope.row.userId)">修改</el-button>
          <el-button type="danger" :icon="Delete" @click="DeleteClick(scope.row.userId)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { getCustomerInfo, getTheCustomerInfo, deleteTheCustomerInfo, modifyTheCustomerInfo, addTheCustomerInfo } from '@/utils/api/user'
...
const muploadAction = (file: File) => {
  ...
  const url = 'https://mock.apifox.cn/m1/2794549-0-default/User/modifyUploadPic';
  try {
    axios.post('https://mock.apifox.cn/m1/2794549-0-default/User/modifyUploadPic', formData, {
      headers: {
...
const uploadAction = (file: File) => {
  let formData = new FormData();
  formData.append('file', file)
  const url = 'https://mock.apifox.cn/m1/2794549-0-default/User/addUploadPic';
  try {
    axios.post('https://mock.apifox.cn/m1/2794549-0-default/User/addUploadPic', formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    })
