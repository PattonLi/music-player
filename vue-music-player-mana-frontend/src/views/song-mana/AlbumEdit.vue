  ...
  <el-card class="main-card">
    <el-table :data="state.tableData" height="600" style="width: 100%" @cell-dblclick="DBClick">
      ...
      <el-table-column fixed="right" label="操作" width="200">
        <template #default="scope">
          <el-button type="warning" :icon="Edit" @click="EditClick(scope.row.albumId)">修改</el-button>
          <el-button type="danger" :icon="Delete" @click="DeleteClick(scope.row.albumId)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  <el-dialog
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="deleteAlbumInfo">
          确认
        </el-button>
      </span>
    </template>
  ...
  <el-dialog
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="artistDialog.visable = false">
          确认
        </el-button>
      </span>
    </template>
...
import { getArtistInfo, getTheArtistInfo, deleteTheArtistInfo, addArtistInfo, modifyArtistInfo, getArtistByAlbum } from '@/utils/api/artist'
import { getArtistAllSong,  deleteTheSongInfo, modifySongInfo, getSongByAlbum } from '@/utils/api/song'
import { getArtistAllAlbum, getAlbumInfo, getTheAlbumInfo, deleteTheAlbumInfo, addAlbumInfo, modifyAlbumInfo } from '@/utils/api/album'
...
const DBClick = (row: AlbumInfo, cell: any, column: any) => {
  if(cell.property === "album") {
    ...
    getArtistByAlbum(row.artistId).then((data) => {
    artistDialog.visable = true
    artistDialog.tableData = data.data
    console.log(artistDialog.tableData)
  })
